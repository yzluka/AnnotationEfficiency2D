...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if main_process():
        max_mIoU_list = [0.,0.,0.]
        best_epoch_list = [-2,-1,0]
        for dummy_epoch in best_epoch_list:
            filename = args.save_path + '/train_epoch_' + str(dummy_epoch) + '.pth'
            print(filename)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.evaluate:
            ...
            if main_process():
                ...
                writer.flush()
                #if mIoU_val > max_mIoU_list[0]:
                if mIoU_val > min(max_mIoU_list):
                    kick_idx = np.argmin(max_mIoU_list)
                    max_mIoU_list.pop(kick_idx)
                    deletename = args.save_path + '/train_epoch_' + str(best_epoch_list.pop(kick_idx)) + '.pth'
                    os.remove(deletename)

                    best_epoch_list.append(epoch_log)
                    max_mIoU_list.append(mIoU_val)
